...
def PLUGIN_SETUP():
    ...
    print(f"🌤️ Weather Plugin v{PLUGIN_METADATA['version']} loaded successfully!")
    return True
...
'''
            sample_plugin.write_text(sample_code, encoding='utf-8')
            print(f"📁 Created sample plugin: {sample_plugin.name}")
    ...
    def load_all_plugins(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        print(f"\n🔌 Loading plugins from: {self.plugins_folder}")
        ...
        if not plugin_files:
            print("📭 No plugins found in plugins folder")
            return self.plugins
        ...
        print(f"✅ Loaded {loaded_count}/{len(plugin_files)} plugins")
        return self.plugins
    ...
    def _load_single_plugin(self, plugin_file: Path, plugin_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not hasattr(module, 'PLUGIN_TRIGGERS'):
                print(f"❌ Plugin {plugin_name} missing PLUGIN_TRIGGERS")
                return None
            
            if not hasattr(module, 'PLUGIN_FUNCTION'):
                print(f"❌ Plugin {plugin_name} missing PLUGIN_FUNCTION")
                return None
            ...
            if not isinstance(triggers, list) or not triggers:
                print(f"❌ Plugin {plugin_name} has invalid triggers")
                return None
            
            # Validate function
            if not callable(plugin_function):
                print(f"❌ Plugin {plugin_name} has invalid PLUGIN_FUNCTION")
                return None
            ...
            for trigger in triggers:
                ...
                if plugin_name not in self.triggers_map[trigger_lower]:
            ...
            if setup_func and callable(setup_func):
                try:
                    ...
                    if setup_success is not False:
                        print(f"   ✅ {plugin_name} setup completed")
                except Exception as e:
                    print(f"   ⚠️ {plugin_name} setup failed: {e}")
            ...
            print(f"   ✅ {plugin_data['name']} v{plugin_data['version']} - {plugin_data['description']}")
    ...
    def check_triggers(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        # Check for exact triggers first (most specific)
        for trigger, plugin_names in self.triggers_map.items():
            # Check if trigger is in text (word boundary aware)
        ...
        # Check for partial matches (triggers in text)
        for trigger, plugin_names in self.triggers_map.items():
    ...
    def get_plugin_info(self, plugin_name: str) -> Optional[Dict[str, Any]]:
        ...
        """
        if plugin_name in self.plugins:
            plugin_data = self.plugins[plugin_name].copy()
            # Add function signature if available
            if plugin_name in self.plugin_functions:
                try:
                    sig = inspect.signature(self.plugin_functions[plugin_name])
                    plugin_data['signature'] = str(sig)
                except:
                    plugin_data['signature'] = "Unknown"
            return plugin_data
        return None
    
    def search_plugins(self, query: str) -> List[Dict[str, Any]]:
        """
        Search plugins by name, description, category, or trigger.
        ...
        for plugin_name, plugin_data in self.plugins.items():
            ...
            if (query in plugin_name.lower() or
                query in plugin_data['name'].lower() or
                query in plugin_data['description'].lower() or
                query in plugin_data['category'].lower() or
                any(query in trigger.lower() for trigger in plugin_data['triggers'])):
                
                results.append({
                    'name': plugin_data['name'],
                    'description': plugin_data['description'],
                    'category': plugin_data['category'],
                    'triggers': plugin_data['triggers'][:5],  # First 5 triggers
...
def search_plugins(query: str) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of matching plugins
    """
    return plugin_manager.search_plugins(query)
