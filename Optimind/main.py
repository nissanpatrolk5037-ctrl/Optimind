...
import argparse
from live_camera import live_camera, image_obj_detect_and_recognition
...
# ================= WHISPER STT CONFIG =================
WHISPER_EXE = Path("E:/Optimind/whisper-cpp/whisper-cli.exe")
MODEL_DIR = Path("E://Optimind//Models//whisper-models//")
...
if get_ram() <= 8:
    MODELS = {
        ...
        "large": MODEL_DIR / "ggml-large-v3-turbo.bin",
    }

if not get_ram() <= 8:
    MODELS = {
        "tiny": MODEL_DIR / "ggml-small.en.bin",
        "small": MODEL_DIR / "ggml-base.en.bin", 
        "base": MODEL_DIR / "ggml-medium.en.bin",
        "medium": MODEL_DIR / "ggml-large-v3-turbo.bin",
...
MODEL_SELECTION = {
    ...
    (21, 40): "medium",  # 21-40 seconds
    (41, float("inf")): "large",   # 41-60 seconds
}
    ...
    for (min_dur, max_dur), model_name in MODEL_SELECTION.items():
        if min_dur <= duration <= max_dur:
            ...
            if model_path.exists():
                print(f"📊 Using {model_name} model for {duration:.1f}s audio")
                break
    else:
        # Fallback to medium
...
class Media():
    ...
    def analyze_and_report():
        ...
        try:
            ...
            return f"✅ Report saved as: {report_file}"
...
def listener_worker():
    ...
    if check_internet_connection():
        print("✅ Online mode: Using Groq API for responses")
    else:
        print("📴 Offline mode: Using local LLM for responses")
    
    # Load plugins
    plugin_manager.load_all_plugins()
    
    print("🎙️ Whisper STT with adaptive models loaded")
    print("Models: tiny (0-5s), small (6-10s), base (11-20s), medium (21s+)")
    print("🔌 Plugins loaded: Press 'p' to manage plugins")
    print("\nStart speaking...\n")
        ...
        try:
            ...
            if energy > VOICE_THRESHOLD:
                if not is_speaking:
                    ...
                    print(f"\r🎤 Listening... {duration:.1f}s", end="", flush=True)
                    ...
                    if duration >= MIN_SPEECH_SECONDS:
                        ...
                        if text:
                            ...
                            if any(trigger in text.lower() for trigger in calendar_triggers):
                                print("📅 Scheduling Google Calendar event...")
                            ...
                            if any(trigger in text.lower() for trigger in telegram_triggers):
                                ...
                                print("❌ Telegram integration not available yet")
                                triggered = True
                            ...
                            if any(trigger in text.lower() for trigger in docker_triggers):
                                print("🐳 Controlling Docker container...")
                            ...
                            if any(trigger in text.lower() for trigger in encrypt_triggers):
                                print("🔒 Encrypting/decrypting file...")
                            ...
                            if any(trigger in text.lower() for trigger in live_camera_triggers):
                                print("Live Object Detection & Recognition")
                                live_camera()

                            if any(trigger in text.lower() for trigger in image_object_detection_triggers):
                                image_path = input("Enter image path: ")
                                result_image, detections = image_obj_detect_and_recognition(image_path)
                            ...
                            if not triggered:
                                ...
                                if plugin_info:
                                    ...
                                    print(f"🔌 Plugin triggered: {plugin_data['name']}")
...
def main():
    ...
    for model_name, model_path in MODELS.items():
        if not model_path.exists():
            print(f"⚠️ Warning: Model {model_name} not found at {model_path}")
